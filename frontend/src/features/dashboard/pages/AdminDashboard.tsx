...
import { bikeApi } from '../../bikes/api/bike.api';
import type { Bike as BikeType } from '../../bikes/bike.types';
...
const AdminDashboard: React.FC = () => {
  ...
  const handleAddBike = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) return;
      await bikeApi.addBike(currentBike, token);
      setShowModal(false);
      fetchBikes();
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this bike?')) {
      try {
        const token = localStorage.getItem('token');
        if (!token) return;
        await bikeApi.deleteBike(id, token);
        fetchBikes();
      } catch (error) {
